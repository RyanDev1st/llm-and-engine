...
- **QUALITY FIX (2026-06-15, commit 18aebb06):** dropped goal substitution from `think_answer` — it inlined verb-phrase goals as the object of "answer" → "time to answer decide between the options directly" (broken grammar) in ~10,064 trained rows (hidden `<think>`, serve-stripped but trained-on). Now goal-free (objective already in the leading `<goal>`). Re-regenerated: **train=73,633 val=1,427**, GATE PASS (max seq **1634**), broken-grammar think now **0 of 108,422** think turns. THIS is the corpus to train on (supersedes 008a01df).
- **TOMORROW:** launch the full E4B Kaggle run (`kaggle_e4b_qlora.ipynb`, MODEL=gemma4_e4b, DATA_STEM=v1_2, MAX_SEQ=1664). Pull commits fresh from GitHub. Notebook GPU-comment fixes DONE (Cell 1+2 now correctly say E4B fits ONE T4 ~10GB; T4×2 only for ~2× DDP speed).
  - **DDP go/no-go = Cell 6.55 A/B** (real 2-proc trainer, 8 steps). The ONE unverified integration point is `acc.prepare()` DDP-wrapping the Unsloth 4-bit + gradient-checkpointed model (can fail with find_unused_parameters / static_graph). If the A/B prints a 2-GPU s/step and a speedup → **DDP=True, MAX_STEPS≈2283** (~1 epoch). If it errors/OOMs or you only get 1 GPU → **DDP=False, MAX_STEPS≈4566** (~1 epoch single-GPU). BOTH are full real training; DDP only buys ~1.8× wall-clock.
  - Multi-session resume: SAVE_EVERY checkpoints; download Cell-8 zip → re-upload as Dataset → RESUME=True next session (any account).
