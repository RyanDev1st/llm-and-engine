...
| [reference/native-gemma-format.md](reference/native-gemma-format.md) | **v5-native wire format** — the Gemma-4 native tool-call/thinking token grammar (single-token ids, structured-message rendering, role=tool survival, strip_thinking, loss-mask rule). The contract train+serve depend on for the native swap |
...
| Doc | What it is |
| --- | --- |
| [report/README.md](report/README.md) | **START HERE for the presentation.** Curated reading guide over all the session's results: the three numbers that matter (routing 88.7% vs 49.6% base; OOD completion 91.7%/95%; tool FP 55→7), what to read vs skip, every number with its honesty caveat, the image inventory, and the PENDING Kaggle-run gap list (P1 chess completion at parity, P2 representative chats, P3 cross-model chart, P4 GGUF quants, P5 E2B). Points to the findings docs, does not duplicate them |
| [findings/2026-06-25-kaggle-chat-ab-and-board-state-fix.md](findings/2026-06-25-kaggle-chat-ab-and-board-state-fix.md) | The 2026-06-25 Kaggle chat A/B (board_on vs board_off, E4B v4): **confirms board_off is apter — the injected LIVE BOARD makes the model hallucinate off-distribution**. Two fixes landed (CPU-tested): (1) `board_state` returned EMPTY because the model emitted `fields=<['all']>` (schema+list junk) and the parser matched it literally — that empty result also caused board_off to loop and play an unrequested extra move; fixed with a liberal field parse + never-empty fallback. (2) Board hook default flipped OFF (`_BOARD_HOOK` + serve notebook) so LIVE matches training. `board_state` working ⇒ board_off both grounds and stops |
| [findings/2026-06-25-train-serve-parity-audit.md](findings/2026-06-25-train-serve-parity-audit.md) | No-retrain audit: the harness, not the weights, caps chess metrics. The completion eval runs the full loop with the LIVE BOARD hook ON (off-distribution; the model trained with NO board line), making `board_state` redundant → the model skips it → `completed` fails despite correct routing = the 13.5% artifact. PROVEN: with `CHESS_BOARD_HOOK=0` the completion-loop prompt is byte-identical to the routing eval + training (full parity). Fix = board hook off for the completion cell (no retrain). Also gates off the precedence line v4 never trained on (`CHESS_PRECEDENCE_RULE`). G/H + the 98.5% are not harness bugs (name-slip the loop recovers; skill-first first-action). Present completion-at-parity + the base→adapter delta |
